...
namespace cs296
{
  dominos_t::dominos_t()
  {
	    ...
	    {	  
			...
			myBodyDef.position.Set(17, 33.52); //set the starting position
			...
			myBodyDef.position.Set(18, 33.52);
			...
			myBodyDef.position.Set(19, 33.52);
			...
			myBodyDef.position.Set(15, 33.52);
    ...
    {   float xm,ym;
        ...
        b2Body* ground4;
        {
  	...
  	
  	    {
		    ...
		    
		    {
		    	...
		    	dynamicBody->CreateFixture(&boxFixtureDef);
		    	dynamicBody->SetAngularVelocity( 180 * DEGTORAD );
		    }
		    
		    {
		        ...
		        b2BodyDef myBodyDef;
		    	myBodyDef.type = b2_kinematicBody; //this will be a dynamic body
		    	myBodyDef.position.Set(xm+width/2.0, ym-1.9+1.4); //set the starting position
		    	myBodyDef.angle = 0; //set the starting angle
		    	b2Body* dynamicBody = m_world->CreateBody(&myBodyDef);
		    	b2CircleShape circle;
                circle.m_radius = width/2 + 0.1;
	    ...
	    
	    {
            ...
            myFixtureDef.restitution = 0;
      		...
      		polygonShape.SetAsBox(4.0f, 0.01f, b2Vec2(-3.9,0), 0 );
            
      		ground1->CreateFixture(&myFixtureDef);   
	        polygonShape.SetAsBox(4.0f, 0.01f, b2Vec2(+3.9,0), 0 );
      		ground4->CreateFixture(&myFixtureDef);   
      		polygonShape.SetAsBox(0.01f, 2.0f, b2Vec2(8.0,0.0), -0.5 );
      		ground4->CreateFixture(&myFixtureDef);   
      		polygonShape.SetAsBox(0.01f, 2.0f, b2Vec2(-8.0,0.0), -0.5 );
	        ground1->CreateFixture(&myFixtureDef);   
	    }
